...
const Dashboard = () => {
  ...
  const [profile] = useState({
    ...
    earningsHistory: [
      { source: 'task', amount: 200, date: '2023-05-15', description: 'Task #1234' },
      { source: 'task', amount: 200, date: '2023-05-14', description: 'Task #1233' },
      { source: 'referral', amount: 1000, date: '2023-05-14', description: 'Referral bonus' },
      { source: 'task', amount: 200, date: '2023-05-13', description: 'Task #1232' },
      { source: 'task', amount: 200, date: '2023-05-12', description: 'Task #1231' },
      { source: 'bonus', amount: 500, date: '2023-05-10', description: 'Weekly bonus' },
    ],
    paymentHistory: [
  ...
  const progressToNext = nextLevel ? Math.min(100, Math.round((profile.investment / nextLevel.investment) * 100)) : 100;
  
  // Recent activity
  const recentActivity = [
    ...earningsHistory.slice(-3).map(e => ({ 
      type: e.source === 'task' ? 'Task' : e.source === 'referral' ? 'Referral' : 'Bonus', 
      desc: e.description || '', 
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] py-12">
      <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-3xl w-full border border-gray-100">
        <h1 className="text-4xl font-bold mb-4 text-blue-900">Welcome, {profile.name}!</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <ul className="text-gray-700 space-y-1 mb-4">
              ...
              <li>From Referrals: <span className="font-bold">KSh {referralEarnings}</span></li>
            ...
            <ul className="text-gray-700 space-y-1">
              <li>Total Tasks Completed: <span className="font-bold">{tasksCompleted}</span></li>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <div className="flex items-center space-x-2">
            <span className="bg-gray-100 px-3 py-1 rounded text-gray-700 select-all">
              https://zenithagency.com/ref/{profile.name.toLowerCase().replace(' ', '-')}
            </span>
            <button
              className="bg-yellow-400 text-blue-900 px-3 py-1 rounded font-semibold hover:bg-yellow-300 transition-colors shadow"
              onClick={() => {
                navigator.clipboard.writeText(`https://zenithagency.com/ref/${profile.name.toLowerCase().replace(' ', '-')}`);
